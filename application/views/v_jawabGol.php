...
<h3 style="text-align: center;">Pilih Golongan</h3>

<!--<form id="acount-infor" method="post" action="<?php echo base_url('index.php/QNA/jawabSubGol'); ?>" class="acount-infor add-admin">
    <?php
    if( ! empty($gol)){ // Jika data siswa tidak sama dengan kosong, artinya jika data siswa ada
      foreach($gol as $data){
        echo "<input type='checkbox' name='check_list[]' alt='heckbox' value='".$data->kode_golongan."'>".$data->judul_golongan."</input></br>";
      }
    }
    ?>
...
<div class="containertable" style="margin: 0 auto;width:70%">
  <div class="row">
    <div class="col-12">
    <form id="acount-infor" method="post" action="<?php echo base_url('index.php/QNA/jawabSubGol'); ?>" class="acount-infor add-admin formkategori">  
    <table id="table" data-toggle="table"  data-search="true">
        <thead class="thead-dark">
          <tr>
            ...
            <th scope="col">Deskripsi Golongan</th>
          </tr>
        </thead>
        <tbody>
          <?php
            if( ! empty($gol)){ // Jika data siswa tidak sama dengan kosong, artinya jika data siswa ada
              foreach($gol as $data){
                  echo "<tr>";
                  echo "<td align='center'><input type='checkbox' name='check_list[]' alt='checkbox' value='".$data->kode_golongan."'></td>";
                  echo "<td>".$data->kode_golongan."</td>";
                  echo "<td>".$data->judul_golongan."</td>";
                  echo "<td>".$data->deskripsi_golongan."</td>";
                  echo "</tr>";
              }
            }
          ?>
